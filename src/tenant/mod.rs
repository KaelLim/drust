...
pub fn build_tenant_router(state: TenantStack) -> Router {
    ...
    let core = Router::new()
        ...
        .route(
            "/t/{tenant}/collections/{coll}/audit",
            put(records::put_audit_handler),
        )
        .route(
        ...
        .route(
            "/t/{tenant}/collections/{coll}/records:upsert",
            post({
                let b = bus.clone();
                let wh = webhooks.clone();
                let fns = functions.clone();
    ...
    let files_router = if let Some(files_state) = state.files {
        ...
        Router::new()
            ...
            // v1.42 — file-caps gate, INNER to bearer_auth (applied BEFORE the
            // bearer layer in source, so axum runs it AFTER bearer on the request
            // path when TenantRef + TenantFileCaps are in extensions). Service is
            // unrestricted; anon/user are gated per-verb against the tenant's
