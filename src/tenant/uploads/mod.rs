...
fn hname(s: &'static str) -> HeaderName {
...
pub async fn create(
    ...
    let total_length = match headers
        .get("upload-length")
    ...
    if total_length as u64 > state.large_upload_max_bytes as u64 {
        return json_error(
            ...
            "UPLOAD_TOO_LARGE",
            &format!(
                "Upload-Length {total_length} exceeds limit {}",
                state.large_upload_max_bytes
            ),
        );
    }
...
pub async fn head(
    ...
    h.insert(
        hname("upload-length"),
        sess.total_length.to_string().parse().unwrap(),
    );
    h.insert(
...
async fn finalize_and_respond(
    ...
    let visibility = if sess.visibility == "public" {
        Visibility::Public
    } else {
        Visibility::Private
    };
