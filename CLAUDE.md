...
> [!IMPORTANT]
> **User tokens (`drust_user_*`) cannot use `/query`, `/query/explain`, or `/mcp`** — drust does not rewrite user-supplied SQL, so row-level `owner_field` filters cannot be enforced on those surfaces. Reject with 403 QUERY_USER_DENIED / MCP_USER_DENIED. For per-user SELECTs against owner-scoped data, expose a stored RPC with `:user_id` (auto-bound from `AuthCtx`). For testing, use a service token. Documented in [`docs/superpowers/specs/2026-05-09-drust-user-auth-design.md`](../docs/superpowers/specs/2026-05-09-drust-user-auth-design.md) §1.
