...
#[tokio::test]
async fn set_anon_caps_round_trip_and_describe_reflects_it() {
    use drust::mcp::tools::exploration::describe_collection as describe_mcp;
    let d = tempfile::tempdir().unwrap();
    let s = svc(&d).await;
    create_collection(
    ...
    let caps0: Vec<String> = d0["anon_caps"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let resp = set_anon_caps(&s, "posts", &[DmlVerb::Insert, DmlVerb::Update])
        .await
        .unwrap();
    assert_eq!(resp["ok"], true);
    let returned: Vec<String> = resp["anon_caps"]
        .as_array()
        .unwrap()
        .iter()
        .map(|v| v.as_str().unwrap().to_string())
    ...
    let caps1: Vec<String> = d1["anon_caps"]
        .as_array()
        .unwrap()
        .iter()
        .map(|v| v.as_str().unwrap().to_string())
    ...
    // Empty caps locks anon out completely.
    let resp = set_anon_caps(&s, "posts", &[]).await.unwrap();
    assert_eq!(resp["anon_caps"].as_array().unwrap().len(), 0);
    let d2 = describe_mcp(&s, "posts").await.unwrap();
    assert!(d2["anon_caps"].as_array().unwrap().is_empty());
}

#[tokio::test]
async fn set_anon_caps_rejects_system_prefix() {
    let d = tempfile::tempdir().unwrap();
    let s = svc(&d).await;
    let err = set_anon_caps(&s, "_system_files", &[DmlVerb::Select])
        .await
        .unwrap_err();
    let msg = err.to_string();
    assert!(
...
#[tokio::test]
async fn set_anon_caps_rejects_unknown_collection() {
    let d = tempfile::tempdir().unwrap();
    let s = svc(&d).await;
