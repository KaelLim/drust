...
async fn recent_writes_filters_tenant_and_excludes_reads() {
    ...
    .await
    .unwrap(); // read op — must be filtered out
    w.send_backfill(mk(
    ...
    assert_eq!(
        rows.len(),
        1,
        "expected exactly the acme insert_record entry; got {} rows",
        rows.len()
    );
...
async fn recent_writes_collection_filter() {
    ...
    let read_conn = open_audit_db_read(&path).unwrap();
    let arc_read = Arc::new(Mutex::new(read_conn));
    let rows = query_recent(&arc_read, "acme", 50, Some("posts"), None)
        .await
        .unwrap();
...
async fn recent_writes_since_ts_filter() {
    ...
    let read_conn = open_audit_db_read(&path).unwrap();
    let arc_read = Arc::new(Mutex::new(read_conn));
    let rows = query_recent(
        &arc_read,
        "acme",
        50,
        None,
        Some("2026-05-25T02:00:00.000Z"),
    )
    .await
    .unwrap();
