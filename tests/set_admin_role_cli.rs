...
fn rejects_bogus_role() {
    ...
    let out = Command::new(exe)
        .env("DRUST_DATA_DIR", tmp.path())
        .args(["--email", "k@x", "--role", "bogus"])
        .output()
        .unwrap();
    assert!(
        !out.status.success(),
        "bogus role must be rejected, but the CLI succeeded"
    );

    // The role must remain unchanged after a rejected run.
    let conn = rusqlite::Connection::open(&meta_path).unwrap();
    let role: String = conn
        .query_row("SELECT role FROM admins WHERE email='k@x'", [], |r| {
            r.get(0)
