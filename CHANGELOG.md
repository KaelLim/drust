...
No new features, no migrations, no new tables, and **no new MCP tools** (still 70). Every item corrects an existing mechanism to match the semantics it already claimed. Three of them consolidate parallel enforcement into a single choke point instead of adding an Nth parallel check. Spec: `docs/superpowers/specs/2026-08-02-audit-p1-round-design.md`.

One finding was not theoretical: **the live host had dropped 962 audit rows over two days**, silently, and the counter that recorded it was the only evidence.

- **The audit writer stopped dropping rows during retention.** The nightly retention pass issued one unbounded `DELETE` against a 1.6 GB audit database and held the writer for its whole duration; inbound entries hit a full channel and were counted into `drust_audit_drops_total` rather than written. Retention is now chunked (`delete_expired_chunk`, 10 000 rows per pass via a rowid subquery — `DELETE … LIMIT` needs a compile flag SQLite is not built with here), the writer drains between chunks, and `journal_size_limit` bounds the WAL that had grown to 1.5 GB. The drain loop's original form matched only `WriterCmd::Insert` and discarded everything else — which would have thrown away the `SetMeta("last_vacuum_ts")` that arrives immediately after `send_retention`, leaving `should_vacuum` permanently seeing `None` and **VACUUMing a 1.6 GB database every night instead of monthly**. Caught by the implementer reading the plan's own code against `main.rs`, before it shipped.
- **Background paths stop resurrecting soft-deleted tenants.** `soft_delete_tenant` renames the tenant directory into `_trash` and evicts the pool, but seven background call sites opened tenants with `get_or_open`, which *creates* on miss — rebuilding `tenants/<id>` outside `_trash`, where no janitor sweeps, and then serving against the fresh empty database. Renamed to `get_or_create` so the name states the hazard, with the background sites moved to `get_if_live`. Review found an **eighth site the enumeration missed**: the auth-cache *hit* branch of `bearer_auth_layer`, which skips the bearer CTE and therefore skips the only `deleted_at IS NULL` filter in the request. Both cache arms now use `get_if_live` and 404 on `None`.
- **The admin plane denies framing.** `X-Frame-Options: DENY` plus CSP `frame-ancestors 'none'` via one small layer. Review caught that `signed_router` is merged *inside* the guarded router, so the first version leaked the guard onto the tenant-facing `/s/t/{tenant}/{key}` route — signed tenant bytes are legitimately embeddable and must not inherit the admin plane's posture.
- **`sqlite_*` internals are write-protected, case-insensitively.** SQLite treats table names case-insensitively; `is_protected_collection` did not, so `SQLite_master` slipped past a check that `sqlite_master` failed. The authorizer's private duplicate of this check is gone — one predicate, both callers.
- **`owner_field` is enforced at the shared per-row insert body.** The check lived at the callers; it now lives in `insert_row_in_tx`, which every structured insert routes through. Its reach into `CallerCtx::Privileged` god-mode is pinned in a comment, because that is the one path where the guard's presence is a deliberate choice rather than an oversight.
- **An UPDATE may grow a tenant, but not past its cap.** UPDATEs were blanket-exempt from `check_tenant_quota` on the reasoning that a shrink must never be blocked — which also permitted unbounded growth by repeated overwrite. `quota::check_update_growth` now refuses only when the write BOTH grows the tenant AND leaves it over the cap, at all three update sites. **The gate is the last thing in the write transaction, after `record_history::capture`** — review found the first version measured before capture, and since the history row carries the full old *and* new images and audit is on by default, that reading misses roughly 2× the payload; a same-length overwrite moves no data pages at all, so the pre-capture measurement reads `after == before` while the transaction commits unbounded growth.
- **The `cron` resource stops leaking the SQL it points at.** `redact_cron` stripped `payload_json` but not `last_error`, and rusqlite's `SqlInputError` renders as `"{msg} in {sql} at offset {n}"` — so a failed cron job published its target RPC's entire statement into an auto-fetchable MCP resource. Now replaced with a boolean marker.
- **…and reviewing that fix found the same mechanism reachable by an anonymous caller, which is strictly worse.** A stored RPC marked `anon_callable` whose column is later dropped returns `400 {"message":"no such column: status in SELECT count(*) FROM events WHERE api_key = 'sk-live-9f2c' AND status = :st at offset 63"}` — the full statement, including anything hardcoded in it, to an unauthenticated caller. The `cron` leak was service-only, and a service token can already read RPC SQL via `list_rpc`; anon cannot. Both arms (`rpc/handler.rs` read, `rpc/exec_write.rs` write) now stringify through `error::sqlite_error_without_sql`. Confirmed by reverting the fix and reproducing the predicted message byte-for-byte, offset included.
- **Batch webhook fan-out reads subscriptions once per batch, not once per row.** A 1000-row batch opened `meta.sqlite` 1000 times to answer the same question. Review then caught that neither new test actually pinned the change — both passed with the production half reverted, because the `subs.is_empty()` early return alone drove the counter to zero. Replaced with a test that fails on revert.
- **The egress allowlist is checked per attempt, not per event.** A delivery retries over ~36 s; removing an origin from the allowlist mid-retry did not stop the remaining attempts. The check moved inside the retry loop. Review then split two cases the first version had collapsed: a **soft-deleted tenant** must revoke egress (fail closed), while a **transient `meta.sqlite` read failure** must not — the original code made the two indistinguishable and terminal, permanently dropping an event for an origin that was still allowlisted, and writing a false reason into the delivery record.
- **Recycling a tenant id no longer destroys the recovery snapshot.** The id-recycle branch purged `_trash/<id>-*` before inserting, so re-creating a deleted tenant's id deleted the very copy the recovery window exists for. Expiry moved in-process rather than depending on `deploy/drust-janitor.sh`, which does not exist in the Docker/compose deployment — for GHCR users the removed purge was the only thing sweeping `_trash` at all.
- **…and that branch was hard-deleting other admins' tenants with no ownership check (HIGH).** `POST /admin/api/tenants` carries no `{id}`, so `tenant_ownership_layer` passes it through with nothing checked, and there was no outer guard to inherit. A `member` who learned a foreign tenant id — they are not secret; they appear in `/public/<tenant-id>/<key>` URLs — could hard-DELETE that tenant's row and tokens mid-recovery-window and take the id permanently, since the un-delete needs the row that just went away. The branch now asks `tenant_access_for`, making it the **seventh site sharing that predicate** (invariant 7). A denied caller gets the same message as a live-id collision, so an invisible tenant does not report *why* the id is unavailable. Fixed alongside a same-second snapshot collision the preserve-`_trash` change had newly introduced: `delete → create → delete` inside one second aimed `rename(2)` at an existing non-empty directory, `let _ =` discarded the ENOTEMPTY, and the handler returned 204 with a live `data.sqlite` no janitor sweeps.
- **`recent_writes` returns the 100 rows it advertises.** The tool description and the MCP prologue both promised the last 100 mutations while the handler defaulted to 50. Review caught that the prose sites were enumerated but the only **machine-readable** one was missed: `RecentWritesArgs::limit`'s doc comment feeds schemars, which publishes `"1..=200; defaults to 50"` into the `tools/list` inputSchema the model actually reads.
- **`tenant_cap_requests` gets retention and a per-admin daily submit limit (#920).** Retention is anchored on `decided_at`, not `created_at` — review caught that anchoring on creation destroys any request that sat pending longer than the window at the first prune after it is decided, giving it zero retention instead of the documented 90 days. And the sibling queue `quota_requests` — the parallel site of everything this adds — had neither retention nor a submit limit, and `remove_admin` cleaned up one queue but not the other, leaving live the recycled-rowid misattribution the v1.57 review had fixed for the first. Both queues now.

### What the review process actually caught

Every task ran the same shape: implement, then two independent reviewers (codex and an adversarial agent, neither seeing the other's output), then a fix agent that had to **verify each finding itself before acting**. 32 findings; 15 confirmed and fixed; 4 refuted with the disproving line quoted.

Two of the confirmed HIGHs — the anon RPC SQL disclosure and the missing ownership gate on the id-recycle branch — were not in the plan at all; both were found while reviewing an adjacent fix. Neither reviewer track was redundant: codex found the schemars input-schema miss and the soft-delete egress case; the adversarial track found both HIGHs, the post-capture quota ordering, the eighth `get_if_live` site, and the tests that passed with the change reverted.

The most useful refutation was codex's HIGH against the batch fan-out, which claimed the per-batch egress snapshot weakened invariant 6. The fix agent proved from `git show HEAD~1:` that the pre-commit code did exactly the same thing, quoted `.claude/rules/background-jobs.md:52` describing it as a known open v1.58 item, and pointed out that the **next task** owned the fix.

### Also

- **`CLAUDE.md` restructured, 88.9 KB → 17.6 KB** (−69% of always-loaded context), with mechanism moved into eight path-scoped `.claude/rules/*.md` files that load only when a matching file is read. The root cause was append-only editing — 27 releases, no commit had ever retired content, so four statements that contradicted the source were still sitting in required reading, three of them about authorization boundaries. A new `AGENTS.md` gives codex the enumerated invariants it had never been given in any prior review round.
- **The dev profile drops full DWARF** (`debug = "line-tables-only"`). Each of the 229 test binaries statically links the lib plus wasmtime and carried ~176 MB of `.debug_info`; `target/debug` was 55.9 GB of test executables alone. Panic backtraces keep file:line.

## v1.57.0 — 2026-07-30

### Per-member tenant creation cap (default 3, with an owner-approved upgrade request)

Until now **no role had any limit on how many tenants it could create** — both entry points went straight to `make_tenant_inner` with no count check. Every tenant costs a SQLite file, two Garage key prefixes, a stats-sampler row, and a 10 GiB quota tier, so an unbounded member could quietly multiply host footprint. This is a guardrail built *ahead* of onboarding members: the live host has four admins, all `owner`-role, so it constrains nobody on day one. Design: `docs/superpowers/specs/2026-07-30-member-tenant-cap-design.md`.

- **`member` is capped; `owner`/`admin` are not** — they already see every tenant, so creating one is a management act. The role test delegates to `tenant_authz::sees_all_tenants` (lockstep rule) and **fails closed** on an unrecognised role, so a future fourth role cannot gain unlimited creation by omission.
- **The per-admin adjustment is a DELTA, not an absolute ceiling.** `effective_cap = max(0, DRUST_MEMBER_TENANT_CAP + admins.tenant_cap_bonus)`. Storing an absolute would leave previously-adjusted admins behind whenever the global default rises — approve someone to 4 while the default is 3, later raise the default to 10, and they would still sit at 4. With a delta their `+1` becomes 11, so a default change lifts everyone uniformly. Every face still speaks absolute numbers (that is what a person means); the handler converts. A negative delta restricts one admin below the default.
- **Usage is live tenants owned**, so deleting a tenant or transferring its ownership frees a slot — cleanup is never punished. The cap bounds *simultaneous* ownership, not create/delete churn.
- **One enforcement point:** the gate is the first statement of `make_tenant_inner`, the single function both creation entry points call while holding the `meta.sqlite` mutex. Count-read and INSERT are therefore one critical section, a refused create writes nothing (it runs before the id-recycle branch's hard-`DELETE`, so a refusal can never destroy a recoverable tenant), and a future third entry point inherits the check. `403 TENANT_CAP_EXCEEDED`.
- **Request flow, admin-plane only** — no MCP tool and no data-plane route, since a tenant's own key must never influence its owner's allowance. A member files an absolute target plus a reason (self-scoped: no on-behalf-of parameter exists); an owner approves or rejects on a `require_owner_layer` queue; an owner can also direct-set or clear any admin's cap. Approve re-validates that the target is **still** an increase, refusing `409` and leaving the row pending — the guard `quota_admin` already carries for adversarial finding F4, since a stale approval would otherwise silently *lower* an admin an owner had just raised.
- **Over-cap never deletes anything.** Lowering a cap, or demoting an owner who owns many tenants, leaves every existing tenant working; only new creation is refused, and the UI shows `18 / 3` with an over-limit marker.
- **Also fixes a real information leak:** `/admin/tenants` called `build_disk_view()` unconditionally, so a `member` saw the host's physical disk used/total, its free percentage, and the "Free up `/var/lib/garage`" banner — host infrastructure state shown to an admin who should only see their own tenants, the same family as the `/admin/_metrics` finding the 2026-07-29 audit put behind `require_owner_layer`. The stat cards are now role-aware: a member gets owned/cap, their own tenants' usage against their own tenants' summed allowance (one host number is meaningless when tiers differ per tenant), and remaining slots — which is where the request button appears at zero. Computed from columns the list query already reads, so no per-tenant SQLite opens were added.
- **And gives `/admin/quota-requests` a way in:** the 2026-07-29 audit found it had zero inbound links anywhere — its page already set `data-section="quota-requests"` but no nav item matched, so an owner could only reach the queue by typing the URL. Both review queues now have owner-gated sidebar entries.
- New env knob **`DRUST_MEMBER_TENANT_CAP`** (default 3, clamped `0..=100`, restart to apply). New column `admins.tenant_cap_bonus` and table `tenant_cap_requests`, both added idempotently — `NOT NULL DEFAULT 0` needs no backfill, and a test pins that a second boot cannot reset an operator-set value.

### Two review engines, four real bugs, zero overlap

An adversarial agent pass and codex ran independently. Neither found what the other did, and in two cases the fix was to **remove a way to be wrong** rather than add a check:

- The gate accepted the creator's role as a `&str` while discarding the role it had just read from the database, so it could be **lied to** — the plan's claim that a future entry point "cannot forget" the check was false; the mistake had merely moved from forgetting the call to passing the wrong string. The parameter is gone.
- `owner_admin_id: Option<i64>` put the whole gate inside `if let Some(..)`, so `None` skipped it and produced an unowned, uncapped tenant. It is now `i64`, making the bypass unrepresentable — and the compiler immediately caught a call site a batch edit had missed, which an advisory comment would not have.
- The cap lookup collapsed "no such admin" and "read failed" into a permissive default, so approving a **deleted** requester marked the request approved and wrote a success audit row while its `UPDATE` touched nothing. Worse, `admins.id` has no `AUTOINCREMENT`, so SQLite reuses a deleted top rowid and the next invited teammate could inherit the orphaned request. `remove_admin` now deletes a departing admin's requests, the approve path 404s a vanished one, and both UPDATEs assert they affected a row.
- The decide transaction checked the affected-row count on one UPDATE but not the other. Not reachable in-process (one connection, one mutex held across the block) but a refactor of the locking would have turned that asymmetry into a duplicate approval with a false audit row.

The full-suite gate then caught a fifth: `tests/meta_sqlite.rs` pins the fresh-install table list exactly so unintended schema drift fails the build. One finding was **rejected with evidence** — a `# REVIEW (low confidence)` locale comment flagged as a workflow leftover is in fact this file's existing convention, with 27 such comments predating the feature.

## v1.56.3 — 2026-07-30

### Security follow-up — replace the P0-1 downgrade with a serve-time CSP sandbox

v1.56.2 closed the same-origin stored-XSS hole (tenant-uploaded `text/html`/SVG rendering inline on the same origin as the admin UI) by downgrading any script-executing content type to `application/octet-stream` + `attachment`, both at upload ingest and at serve. That was safe but too strong: it broke a real, already-in-use feature — a tenant's Google-Docs-export-to-HTML publishing pipeline could no longer be *viewed*, only downloaded, and SVG logos stopped rendering.

- **Ingest no longer touches the declared/stored content type**, other than lowercasing the essence (`storage::files::normalize_content_type_case` — MIME matching is case-insensitive, so this changes nothing about how any client renders the file). It renders as declared. The safety decision moved to serve time and to the Caddy layer below.
- **Every drust byte responder** (`tenant_files::stream_bytes`, `signed_bytes::respond`, the admin `/admin/files/{key}/bytes` handler) now sends `Content-Security-Policy: sandbox allow-top-navigation-by-user-activation` whenever `storage::files::is_unsafe_inline_type` flags the stored type — via one shared function, `storage::files::insert_content_type_headers`, so the decision cannot drift between the three call sites. Bare `sandbox` gives the rendered document a unique opaque origin and disables scripts/forms/popups; `allow-top-navigation-by-user-activation` restores ordinary hyperlink clicks without re-enabling anything script-driven. This is the same technique GitHub uses to serve arbitrary user HTML from `raw.githubusercontent.com` directly — CSP3 extends `sandbox` to top-level (not just framed) documents, and Chrome/Firefox/Safari all enforce it there.
- **`allow-same-origin` and `allow-scripts` must never be added** to the CSP constant — either one alone defeats the protection. A codified test pins the exact string.
- **The Caddy `/public/*` block** (which bypasses drust entirely, reverse-proxying straight to Garage) got a matching `handle_response` matcher on the *upstream's own served `Content-Type`* — not the request path's file extension, which a caller can set independently of the declared type. This also closes the residual exposure the 2026-07-29 audit found: 63 pre-existing public objects (a docs-publishing tenant's HTML pages, some SVG logos) that predated any fix now get the sandbox CSP too, with no data migration needed. Caddy 2.6.2's response-matcher allowlist has no regex or case-insensitive option — only a plain glob on `header` — so the matcher is boundary-aware exact + prefix-with-`;` pairs per essence (a loose `text/html*` would also, wrongly, flag `text/html-ish`), and drust normalizes essence casing at ingest (above) so this case-sensitive matcher only needs to match one canonical case.
- **Two issues only live testing against the real Caddy binary caught** (design review and `caddy adapt` both missed them — `adapt` only proves a config parses, not that it matches or behaves correctly): (1) Caddy's `handle_response` **replaces** the default passthrough — a block containing only a `header` directive silently discards the entire upstream response body. Caught against a real 2.6 MB object before shipping (200 OK, 0-byte body); fixed by adding `copy_response`. (2) the case-sensitivity gap above, discovered while diagnosing why a deliberately-mixed-case fixture didn't match.
- **Dual-engine review** (an adversarial workflow agent + codex, independently, on the Rust diff). The workflow agent flagged a deployment-ordering hazard (this drust build must not ship before the Caddy `/public/*` change is live — otherwise a public markup upload renders unsandboxed for the gap; addressed by applying the Caddy change first) and a real test-coverage gap (the CSP header was integration-tested on only one of the three responders). The gap was closed structurally rather than by writing two more router harnesses: the duplicated three-line header-insertion pattern was extracted into the one shared `insert_content_type_headers` function, unit-tested once for all three call sites. codex's only finding: the CSP-invariant test checked substrings, which would wave through a widened-but-still-token-free value — strengthened to an exact-literal pin.
...
Per-tenant `/t/<id>/mcp` gains an MCP **Resources** + **Prompts** read surface — hand-written `ServerHandler` methods (rmcp has no resource/prompt macro) that thin-route to the existing `src/mcp/tools/*` readers. **Adds no tools — MCP tool count stays 70.** Service-only by the same `mcp_dispatch` gate; tenant + pool come from the per-tenant `self.state` (no request-context plumbing).

- **Resources** address tenant knowledge as `drust://<tenant>/<path>`. 9 **static** (`resources/list`): `schema`, `schema.md`, `collections`, `openapi.json`, `types.ts`, `zod.ts`, `functions`, `rpcs`, `cron`. 4 **templates** (`resources/templates/list`): `collections/{collection}/schema`, `collections/{collection}/records{?page,per_page,sort,order}`, `collections/{collection}/records/{id}`, `rpcs/{name}`. Bodies capped at `DRUST_MCP_RESOURCE_MAX_BYTES` (default 256 KiB); one `mcp.resource.read` audit row per read.
- **Credential-safe because a resource is auto-fetchable into model context** (unlike a tool that needs an explicit call): `rpcs`/`rpcs/{name}` strip RPC `sql` + param `default`; `cron` strips `payload_json`; the **`history` + `functions/{name}/logs` templates are deliberately NOT resources** (old-row snapshots / function stdout can carry secrets — they stay behind the `get_record_history` / `get_function_logs` tools).
- **URI parser** is a SINGLE `url::Url::parse` + deny-by-default: authority gate + `as_str()==raw` (rejects literal AND `%2e`-encoded dot-segments — both are url-normalized) + no-`%` segments + per-route query hardening (reject `%`/`+`/duplicate/unknown keys — `query_pairs()` form-decodes past `as_str()==raw`, e.g. `?p%61ge=2`→`page`). Cross-tenant guard is `host_str()==tenant_id`; `{c}` runs `identifier` + `is_protected_collection`; `{id}` is a canonical i64 (rejects `/records/05`). `Record{c,id}` uses a dedicated bound-i64 single-row reader (the `/list` filter validator only accepts declared fields).
- **`resource_link`** on read tools: a top-level concrete link on `insert_record`, a `resource_uri_template` on `list_records` + `search_collection` — never per-row (default projection omits `id`, and `resource_link` is a legal user column).
- **Prompts** (`prompts/list` + `prompts/get`): 6 task recipes — `bootstrap` (live `schema.md` + core rules), `design_collection{purpose}`, `secure_collection{collection}`, `debug_write{collection}`, `write_edge_function{trigger}`, `review_history{collection}`. Bodies mirror existing service tools (no new exposure); unknown / missing-arg → `-32602`.
- **`listChanged` is NOT declared. M5 (resources/list_changed notifications) + M6 (subscriptions) are deferred** — both need a per-tenant schema-change notification bus + a live MCP-peer subscription (the `#[tool]` handlers don't receive the peer, and schema also mutates from the REST/admin faces), which is disproportionate infra for a re-listable convenience.
- **Two-engine review per task (codebase-memory confirm → codex cross-check → implement → codex confirm).** codex's design pass (with a compiled `url` v2 probe) caught five real issues before any code shipped: the id-filter approach was unworkable (→ dedicated reader), a query parser-differential (`?p%61ge=2`), a broken per-row `resource_link` (→ top-level), an auto-fetch credential exposure in `history`/`logs` (→ dropped), and a `describe_collection` error-sentinel that never reached the error mapper. Its impl pass returned all findings CONFIRMED-CORRECT.

## v1.55.0 — 2026-07-29

### SQLite Wave 1 — M2: batch insert + upsert

- **`POST /t/<id>/collections/<c>/records:batch` + MCP `insert_records`**, and **`POST .../records:upsert` + MCP `upsert_records`** (MCP tool count 68 → 70). **Service-only on both faces** (REST checks `AuthCtx::Service` → anon/user `403 WRITE_DENIED`; MCP is service-only by dispatch). Both run MANY rows through the SAME per-row guards as a single insert (`write::insert_row_in_tx`) inside ONE `with_writer_tx`, **atomically** — any invalid row rolls back every data row AND every `_system_record_history` row. Quota re-measures per row (`usage_on_conn` reflects in-tx page growth); `DRUST_BATCH_MAX_ROWS` (default 1000) caps rows; `owner_field`-required per row on owner-scoped collections; `_system_*` refused. One primitive-based core (`batch_insert_core` / `batch_upsert_core`) backs both faces.
- **Upsert** = `INSERT ... ON CONFLICT(<cols>) DO UPDATE SET <non-key col>=excluded.<col>, updated_at=datetime('now') RETURNING *`. A **pre-image probe by the conflict key** decides the per-row op (`insert` vs `update`) so record-history captures the correct old/new and the fan-out emits `Created` / `Updated` per row. Quota gates the **insert branch only** (a conflict UPDATE mirrors `update_record_checked`: never block a shrink); the convergent `updated_at` keeps `RETURNING *` equal to the committed post-trigger row (v1.43). **Conflict-target validation queries `PRAGMA index_xinfo`/`index_list`/`table_info` DIRECTLY** (accepts a `UNIQUE` column autoindex, a named unique index, and single/composite/text PK; returns per-column collation), rather than `describe_collection().indices` (which filters autoindexes out). Errors: `UPSERT_NO_UNIQUE` / `UPSERT_MISSING_KEY` / `UPSERT_NO_CONFLICT_COLS` / `UPSERT_DUPLICATE_COLUMN` / `UPSERT_DUPLICATE_IN_BATCH` (all 400).
- codegen: service-tier OpenAPI gains `/records:batch` + `/records:upsert`; `recent_writes` WRITE_OPS gains `insert_records` / `upsert_records`.
- **Two-engine review (codex + an independent adversarial workflow, every finding REFUTE-verified, plus a self-verify pass on the fixes).** No SQL-injection / tenant-isolation / atomicity / service-gating issues. Correctness fixes shipped before release:
  - **collation-correct pre-image probe** — a `NOCASE` (or `RTRIM`) unique index made SQLite take the DO UPDATE branch while a BINARY `=` probe misclassified it as an insert (wrong history op, lost old image, a pure update wrongly quota-gated). The validator now returns the conflict index's per-column collation and the probe applies `COLLATE`.
  - **server-managed columns preserved on update** — the `DO UPDATE SET` excluded `id` / `created_at` (an external-sync payload carrying its own `id` no longer moves the existing row's PK or resets its creation time); the insert branch still honors them.
  - **within-batch duplicate conflict keys rejected** (`UPSERT_DUPLICATE_IN_BATCH`, keyed on the resolved id — matches Postgres "cannot affect row a second time"), so no superseded intermediate `Created` event escapes; **duplicate `on_conflict` columns rejected**; **vector-encode errors on batch/upsert map to 422** for parity with single insert.
  - hardening: `validate_conflict_target` skips **partial** and **expression-tainted** unique indexes (clean `UPSERT_NO_UNIQUE` instead of a confusing runtime error); the within-batch dedup falls back to the raw key tuple for a no-`id` table.
