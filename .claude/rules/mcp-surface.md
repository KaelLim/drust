...
> [!CAUTION]
> **A resource is auto-fetchable into model context (spec §3), unlike a tool that needs an explicit call** — so anything credential-bearing must stay behind a TOOL. `rpcs`/`rpcs/{name}` strip RPC `sql` + param `default`; `cron` strips `payload_json`; the `history` + `functions/{name}/logs` templates are **deliberately NOT exposed as resources** (old-row snapshots / function stdout can carry secrets — they stay behind the `get_record_history` / `get_function_logs` TOOLS). Known leak being fixed in v1.58: `redact_cron` removes `payload_json` but **not `last_error`**, and rusqlite's `SqlInputError` `Display` embeds the whole SQL — so a prepare-failing RPC cron job leaks through `cron` the very SQL the `rpcs` resource strips.
