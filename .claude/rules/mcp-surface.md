...
`src/mcp/{resources,prompts}.rs` — hand-written `ServerHandler` methods; **rmcp has no resource/prompt macro**. Read surface only; all **service-only** by the same `mcp_dispatch` gate, tenant + pool from the per-tenant `self.state` (no request-context plumbing). Resources address tenant knowledge as `drust://<tenant>/<path>`: 10 static (`schema`, `schema.md`, `collections`, `openapi.json`, `types.ts`, `zod.ts`, `functions`, `rpcs`, `cron`) thin-routing to the existing `get_schema_overview`/codegen/inventory readers, plus **`files-guide.md`** (v1.64) — the one static resource that is AUTHORED rather than projected: a Markdown handbook for the upload stations, the public/private model and the per-prefix publish grant, with this tenant's live `public_upload_roles` list appended at read time. It carries no credentials by the rule below (tokens stay in the `whoami` TOOL, which points at it through a two-key `file_upload` block — deliberately a signpost, since the alternative was a `whoami` that grows a paragraph per feature). `mcp::tools::file_policy::public_upload_grants` is the ONE grant lister both surfaces call. Plus 4 templates (`collections/{collection}/schema`, `collections/{collection}/records{?page,per_page,sort,order}`, `collections/{collection}/records/{id}`, `rpcs/{name}`).
