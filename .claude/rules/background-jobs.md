...
The webhook egress gate reaches "per attempt" through **two** reads, not one. `dispatch_many`'s fan-out read gates **attempt 1** of every delivery it spawns — once per batch, which is what keeps a 1000-row batch from opening 1000 meta connections (`tests/batch_webhook_fanout.rs` pins the exact count). The retry loop in `deliver_inner` re-reads the allowlist before **attempts 2..n**, and a denial there is TERMINAL (`DeliveryError::EgressRevoked`, recorded with the same `egress_not_allowlisted: <url>` reason the fan-out gate writes). Before v1.58 only the fan-out read existed, so an origin removed mid-flight kept receiving POSTs for the rest of the ~36 s schedule. Anything that adds an attempt must stay inside that loop or re-check for itself.
