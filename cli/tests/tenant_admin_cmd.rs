...
async fn users_list() {
    ...
    Mock::given(method("GET")).and(path("/t/9f/admin/users"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"users":[]}))).mount(&server).await;
    ...
    cli(tmp.path()).args(["--json","users","list"]).assert().success().stdout(predicate::str::contains("users"));
}
