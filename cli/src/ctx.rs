...
impl Ctx {
    ...
    pub fn build(cli: &Cli, need_tenant: bool) -> anyhow::Result<Ctx> {
        ...
        let host = cfg.hosts.get(&host_key).expect("resolved key exists").clone();
        let token = store::read_token(&host_key, &host)?;
        let tenant = if need_tenant {
            cli.tenant
                .clone()
                .or(host.default_tenant.clone())
                .ok_or_else(|| anyhow::anyhow!("no tenant selected — run 'drust use <tenant>' or pass --tenant"))?
        } else {
            String::new()
        };
        let renderer = Renderer::resolve(cli.json, cli.output.as_deref(), std::io::stdout().is_terminal());
        Ok(Ctx { client: DrustClient::new(host.base_url, token), tenant, renderer, host_key })
    }
}
