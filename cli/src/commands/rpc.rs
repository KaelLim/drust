...
pub async fn run(cli: &Cli, a: &RpcArgs) -> anyhow::Result<i32> {
    ...
    match &a.cmd {
        RpcCmd::Call {
            ...
            let body: serde_json::Value = match params {
            ...
            finish(
                &ctx,
                ctx.client
                    .send_json(Method::POST, &format!("/t/{t}/rpc/{name}{q}"), body)
                    .await,
            )
        }
        RpcCmd::List => match ctx.client.get(&format!("/t/{t}/schema/overview")).await {
            Ok(v) => {
                ctx.renderer
                    .value(v.get("rpcs").unwrap_or(&serde_json::Value::Null));
                Ok(0)
            }
            Err(e) => {
