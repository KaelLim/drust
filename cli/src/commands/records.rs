    ...
    match &a.cmd {
        RecordsCmd::List {
            ...
            if let Some(f) = filter {
                body.insert("filter".into(), serde_json::from_str(f)?);
            }
            if let Some(s) = sort {
            ...
            if let Some(sel) = select {
                body.insert(
                    "select".into(),
                    serde_json::json!(sel.split(',').collect::<Vec<_>>()),
                );
            }
            finish(
                &ctx,
                ctx.client
                    .send_json(
                        Method::POST,
                        &format!("/t/{t}/collections/{coll}/list"),
                        serde_json::Value::Object(body),
                    )
                    .await,
            )
        }
        RecordsCmd::Get { coll, id } => finish(
            &ctx,
            ctx.client.get(&format!("/t/{t}/records/{coll}/{id}")).await,
        ),
        RecordsCmd::Create { coll, data } => finish(
            &ctx,
            ctx.client
                .send_json(
                    Method::POST,
                    &format!("/t/{t}/records/{coll}"),
                    serde_json::json!({"data": serde_json::from_str::<serde_json::Value>(data)?}),
                )
                .await,
        ),
        RecordsCmd::Update { coll, id, data } => finish(
            &ctx,
            ctx.client
                .send_json(
                    Method::PATCH,
...
pub fn finish(
    ctx: &Ctx,
    r: Result<serde_json::Value, crate::client::error::ApiError>,
) -> anyhow::Result<i32> {
    match r {
        Ok(v) => {
            ctx.renderer.value(&v);
            Ok(0)
        }
        Err(e) => {
            ctx.renderer.error(&e);
