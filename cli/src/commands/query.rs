...
pub async fn run(cli: &Cli, a: &QueryArgs) -> anyhow::Result<i32> {
    anyhow::ensure!(
        a.r#unsafe,
        "drust query runs raw SQL (service-only); pass --unsafe to confirm, or use 'records list'/'search'"
    );
    let ctx = Ctx::build(cli, true)?;
    let p = if a.explain {
        format!("/t/{}/query/explain", ctx.tenant)
    } else {
        format!("/t/{}/query", ctx.tenant)
    };
    finish(
        &ctx,
        ctx.client
            .send_json(Method::POST, &p, serde_json::json!({"sql": a.sql}))
            .await,
    )
}
