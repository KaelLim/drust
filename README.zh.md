...
- **Realtime broadcast。** 每個 `(tenant, collection)` 一條 SSE channel（`/t/<id>/records/<c>/subscribe`），collection 層級可開關，受 anon-cap 控管，user token 永久拒絕。
...
- **Outbound webhook。** 每租戶可訂閱 CRUD 事件，POST 載荷帶 HMAC-SHA256 簽章，4 次重試（+0s / +1s / +5s / +30s）；admin UI / REST / MCP 三種寫入面；SSRF 防護擋私網 / loopback IP。
...
- **Admin UI。** 雙頁式 web UI（`/admin/tenants` + 各租戶 `/admin/tenants/<id>/<datatable>`），終端機風格，含檔案管理、RPC 編輯器 + test playground、anon 能力矩陣、MCP 設定範本、audit log 內嵌 SVG 圖表、backup 瀏覽器 + 單租戶還原。
...
- **Admin OAuth。** `/admin/login` 除了帳密之外，也支援 Google / GitHub 登入；email 允許清單由 env 設定。
...
Production。目前版本 `v1.20.0`。完整變更歷史見 [CHANGELOG.md](CHANGELOG.md)。
